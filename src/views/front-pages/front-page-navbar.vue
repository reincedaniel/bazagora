  ...
  >
    ...
    <div>
      <div class="d-flex flex-column gap-y-4 pa-4">
        <RouterLink
          v-for="(item, index) in [
  { target: 'Home', label: 'Início' },
  { target: 'Features', label: 'Quem Somos' },
  { target: 'Team', label: 'Nossa Equipe' },
  { target: 'FAQ', label: 'FAQ' },
  { target: 'Contact us', label: 'Contatos' }
]"
          :key="index"
          :to="{ name: 'root', hash: `#${item.target.toLowerCase().replace(' ', '-')}` }"
          class="nav-link font-weight-medium"
          :class="[props.activeId?.toLocaleLowerCase().replace('-', ' ') === item.target.toLocaleLowerCase() ? 'active-link' : '']"
        >
          {{ item.label }}
        </RouterLink>
